<?php
class ThiEngineParser_PHP extends ThiEngineParser {
	function ThiEngineParser_PHP($text)
	{
		parent::__construct($text);
		
		$this->LANG_NAME = 'PHP';
		$this->SCRIPT_DELIMITERS = array(
	        0 => array(
	            '<?php' => '?>'
	        ),
	        1 => array(
		...
		$this->STRING_DELIMITERS = array(
			0 => array('"', '"', "\\"),
			1 => array("'", "'", "\\"),
			2 => array("<<<", null, null, '(([_a-zA-Z][_a-zA-Z\d]*))\n|\0', '(\n)(/eom/;?)\n'),
			3 => array("<<<", null, null, '(\'([_a-zA-Z][_a-zA-Z\d]*)\')\n|\0', '(\n)(/eom/;?)\n')
		);
		$this->COMMENT_MULTI = array(
			0 => array("/*", "*/")
		);
		$this->COMMENT_SINGLE = array(
			0 => "//", 
			1=> "#"
		);
		$this->HIGHLIGHT_STRICT_BLOCK = array(
			0 => true,
			1 => true,
			2 => true,
			3 => true
		...
		$this->OBJECT_SPLITTERS = array(
			'->', '::'
		);
		
		$this->MEMBER_IDENTIFIER = '[_a-zA-Z][_a-zA-Z\d]*';
		...
		$this->STYLES = array(
			...
			'OBJECT_MEMBER' => 'color: #006600;',
			'OBJECT_SPLITTER' => 'color: #006600;',
			'SYMBOL' => array(
				0 => 'color: #66cc66;'
			),
		);
